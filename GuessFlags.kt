    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            Surface(
                ...
                GuessFlag(navController, timerViewModel)
            }
        }
    }
}
...
@Composable
fun GuessFlag (navController: NavHostController, timerViewModel: TimerViewModel) {
    LaunchedEffect(true) {
    ...
    Column(modifier = Modifier
        ...
        Text(text = "Guess The Flag", style = TextStyle(
            fontSize = 40.sp,
            fontWeight = FontWeight.Bold,
        )
        )
        Text(text = "Timer: ${timerViewModel.liveElapsedTime.value} seconds")
        Flag(timerViewModel = timerViewModel)
    }
...
@Composable
fun Flag(timerViewModel: TimerViewModel = remember { TimerViewModel() }) {
// list of flags and their names
    ...
    // also displays correct answer and that the user had picked a flag
    val randomIndex = remember { mutableIntStateOf(Random.nextInt(flagAndCountries.size)) }
    var selectedFlagAndCountry by remember { mutableStateOf(flagAndCountries[randomIndex.intValue]) }
    var isCorrectGuess by remember { mutableStateOf(false) }
    var userHasMadeSelection by remember { mutableStateOf(false) }
    Text(
        text = "Which flag belongs to ${selectedFlagAndCountry.countryName}?",
        modifier = Modifier
            .padding(10.dp)
            .fillMaxWidth(),
        fontSize = 30.sp,
    ...
    // shuffles the flags and has one correct answer  with a scroll bar
    val options = flagAndCountries.shuffled().take(2) + selectedFlagAndCountry
    val shuffledOptions = if (!userHasMadeSelection) options.shuffled() else options
    Box(
        modifier = Modifier
            .fillMaxWidth()
            .padding(8.dp)
        ...
        Column(
            ...
            shuffledOptions.shuffled().forEach { flagAndCountry ->
                if (!userHasMadeSelection)
                    Image(
                        painter = painterResource(id = flagAndCountry.drawableId),
                        contentDescription = null,
                        modifier = Modifier
                            .clickable {
                                if (!userHasMadeSelection) {
                                    isCorrectGuess = flagAndCountry == selectedFlagAndCountry
...
// displays right or wrong
            if (userHasMadeSelection) {
                Text(
                    text = if (isCorrectGuess) "Correct" else "Wrong",
                    color = if (isCorrectGuess) Color.Green else Color.Red,
                    fontWeight = FontWeight.Bold,
            ...
            Column(
                ...
                Button(
                    onClick = {
                        ...
                        randomIndex.intValue = Random.nextInt(flagAndCountries.size)
                        selectedFlagAndCountry = flagAndCountries[randomIndex.intValue]
